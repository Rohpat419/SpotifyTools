...
**Spotify OAuth:**The spotify OAuth flow is a pain. On both the backend and frontend. Dealing with the success page on the frontend and dealing with the OAuth flow on the backend was a pain.  AI did not do the best here and it took multiple shots and debugging sessions to get this right (more on this later). 

**Lack of staging server:**Without a staging server, testing the code was difficult and caused a lot of code pushes instead of local development. This was a bad idea for a project that kept getting iterated on. 
...
**Logging:**Logging is a developer's best friend. I think the less logging I had when dealing with an OAuth issue, the longer it took to debug. Adding more logging statements and try/except blocks were extremely important in debugging the errors. Or else the errors would be ridiculously opaque. 
